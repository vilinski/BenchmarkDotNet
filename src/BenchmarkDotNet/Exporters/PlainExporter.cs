        ...
        public override void ExportToLog(Summary summary, ILogger logger)
        {
            foreach (var report in summary.Reports)
            {
                ...
                foreach (var run in runs)
                    logger.WriteLineResult(run.ToStr(report.BenchmarkCase.Config.FormatStyle));
                foreach (var (mode, stage) in modeStages)
                {
                    logger.WriteLine();
                    logger.WriteLineHeader($"* Statistics for {mode}{stage}");
                    logger.WriteLineStatistic(runs.Where(it => it.Is(mode, stage)).GetStatistics().ToTimeStr(report.BenchmarkCase.Config.FormatStyle, calcHistogram: true));
                }
            }
        }
    }
